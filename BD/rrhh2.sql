...
INSERT INTO `nomina` VALUES (22,3,'Quincenal','2025-05-19',7200.00,250.00,0.00,540.00,360.00,347.76,7282.24),(23,1,'Mensual','2025-05-19',5750.00,250.00,0.00,539.10,0.00,277.73,6261.37),(24,5,'Mensual','2025-05-19',8000.00,250.00,900.00,0.00,400.00,386.40,8363.60),(25,2,'Semanal','2025-05-19',5800.00,250.00,0.00,271.88,0.00,280.14,6041.74);
...
INSERT INTO `productividad` (`ID_Prod`, `ID_Emp`, `Mes`, `Anio`, `HorasTrabajadas`, `HorasExtras`, `HorasDescanso`) VALUES (1,1,4,2025,160.00,10.00,20.00),(2,2,4,2025,145.00,5.00,15.00),(3,3,4,2025,155.00,8.00,18.00),(4,5,4,2025,140.00,0.00,10.00),(5,2,5,2025,80.00,12.00,10.00);
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `spActualizarProductividad`(
    IN p_ID_Prod INT,
...
BEGIN
    ...
    SET 
        HorasTrabajadas = p_HorasTrabajadas,
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `spEmpleadosSinProductividadMesActual` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'NO_ZERO_IN_DATE,NO_ZERO_DATE,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `spEmpleadosSinProductividadMesActual`()
BEGIN
    SELECT 
        e.ID_Emp,
        CONCAT(e.PriNombre_Emp, ' ', e.SegNombre_Emp, ' ', e.PriApellido_Emp, ' ', e.SegApellido_Emp) AS nombre_completo
    ...
    WHERE e.Estado_Emp = 'Activo'
      AND NOT EXISTS (
          SELECT 1 FROM productividad p
          WHERE p.ID_Emp = e.ID_Emp
          AND p.Mes = MONTH(CURDATE()) AND p.Anio = YEAR(CURDATE())
      );
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `spGenerarProductividadEmpleado` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'NO_ZERO_IN_DATE,NO_ZERO_DATE,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `spGenerarProductividadEmpleado`(
    IN p_ID_Emp INT,
    IN p_Mes INT,
    IN p_Anio INT,
    IN p_HorasTrabajadas DECIMAL(5,2),
...
BEGIN
    ...
    -- Validar que el mes/año no sea anterior a la fecha de ingreso
    SELECT FechaIngreso_Emp INTO ingreso
    FROM empleado
    WHERE ID_Emp = p_ID_Emp;

    IF ingreso IS NULL OR ingreso > LAST_DAY(CONCAT(p_Anio, '-', p_Mes, '-01')) THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'No se puede generar productividad para fechas anteriores al ingreso.';
    END IF;

    -- Validar que no exista duplicado
    IF EXISTS (
        SELECT 1 FROM productividad 
        WHERE ID_Emp = p_ID_Emp AND Mes = p_Mes AND Anio = p_Anio
    ) THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Ya existe productividad para ese mes.';
    END IF;

    -- Insertar productividad
    INSERT INTO productividad (ID_Emp, Mes, Anio, HorasTrabajadas, HorasExtras, HorasDescanso)
    VALUES (p_ID_Emp, p_Mes, p_Anio, p_HorasTrabajadas, p_HorasExtras, p_HorasDescanso);
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
-- Dump completed on 2025-05-20  1:42:20
